...
bool CCollision::TileExists(int Index)
{
	...
	if(m_pFront && m_pFront[Index].m_Index >= TILE_FREEZE && m_pFront[Index].m_Index  <= TILE_NPH_START)
		return true;
	if(m_pTiles[Index].m_Index >= TILE_RAINBOW && m_pTiles[Index].m_Index <= TILE_RMEXTRAS)
		return true;
	if(m_pFront && m_pFront[Index].m_Index >= TILE_RAINBOW && m_pFront[Index].m_Index  <= TILE_RMEXTRAS)
		return true;
