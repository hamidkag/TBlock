...
void CCharacter::Snap(int SnappingClient)
{
	...
	if(GetPlayer()->m_Invisible &&	GetPlayer()->GetCID() != SnappingClient &&	GameServer()->m_apPlayers[SnappingClient]->m_Authed < GetPlayer()->m_Authed)
		return;
...
void CCharacter::HandleTiles(int Index)
{
	...
	//UNVIS
	if((m_TileIndex == TILE_UNVIS) || (m_TileFIndex == TILE_UNVIS))
	{
		if (m_LastIndexTile == TILE_UNVIS || m_LastIndexFrontTile == TILE_UNVIS)
			return;
		...
		{
			m_pPlayer->m_Invisible = false;
			str_format(aBuf, sizeof(aBuf), "visible disabled");
		}
		else{
			str_format(aBuf, sizeof(aBuf), "You Are Not Invis!");
		}
			GameServer()->SendChatTarget(m_pPlayer->GetCID(), aBuf);
	}
	//HAMMER
	...
	//CLANS
	if((m_TileIndex == TILE_RCLAN) || (m_TileFIndex == TILE_RCLAN))
	{
			...
			else
			{
				GameServer()->CreateSound(m_Pos, SOUND_PLAYER_PAIN_LONG);
				Die(m_pPlayer->GetCID(), WEAPON_WORLD);
				GameServer()->SendChatTarget(m_pPlayer->GetCID(), "Your clan name must be [P.G] to enter Clan Room.");
				m_pPlayer->m_ClanMSG = false;
			}
	}
	if((m_TileIndex == TILE_PCLAN) || (m_TileFIndex == TILE_PCLAN))
	{
			if(str_comp(Server()->ClientClan(m_pPlayer->GetCID()), "PersianPro") == 0)
			{
				if(m_pPlayer->m_ClanMSG)
				{
					GameServer()->CreateSound(m_Pos, SOUND_RIFLE_BOUNCE);
					GameServer()->SendChatTarget(m_pPlayer->GetCID(), "Welcome to PersianPro Clan Room..!");
					m_pPlayer->m_ClanMSG = false;
				}
			}
			else
			{
				GameServer()->CreateSound(m_Pos, SOUND_PLAYER_PAIN_LONG);
				Die(m_pPlayer->GetCID(), WEAPON_WORLD);
				GameServer()->SendChatTarget(m_pPlayer->GetCID(), "Your clan name must be PersianPro to enter Clan Room.");
				m_pPlayer->m_ClanMSG = false;
			}
	}
	if((m_TileIndex == TILE_CCLAN) || (m_TileFIndex == TILE_CCLAN))
	{
			if(str_comp(Server()->ClientClan(m_pPlayer->GetCID()), "[C.Z]") == 0)
			{
				if(m_pPlayer->m_ClanMSG)
				{
					GameServer()->CreateSound(m_Pos, SOUND_RIFLE_BOUNCE);
					GameServer()->SendChatTarget(m_pPlayer->GetCID(), "Welcome to CrazyBax Clan Room..!");
					m_pPlayer->m_ClanMSG = false;
				}
			}
			else
			{
				GameServer()->CreateSound(m_Pos, SOUND_PLAYER_PAIN_LONG);
				Die(m_pPlayer->GetCID(), WEAPON_WORLD);
				GameServer()->SendChatTarget(m_pPlayer->GetCID(), "Your clan name must be [C.Z] to enter Clan Room.");
				m_pPlayer->m_ClanMSG = false;
			}
	}
	if((m_TileIndex == TILE_VCLAN) || (m_TileFIndex == TILE_VCLAN))
	{
			if(str_comp(Server()->ClientClan(m_pPlayer->GetCID()), "VeNuM") == 0)
			{
				if(m_pPlayer->m_ClanMSG)
				{
					GameServer()->CreateSound(m_Pos, SOUND_RIFLE_BOUNCE);
					GameServer()->SendChatTarget(m_pPlayer->GetCID(), "Welcome to VeNuM Bax Clan Room..!");
					m_pPlayer->m_ClanMSG = false;
				}
			}
			else
			{
				GameServer()->CreateSound(m_Pos, SOUND_PLAYER_PAIN_LONG);
				Die(m_pPlayer->GetCID(), WEAPON_WORLD);
				GameServer()->SendChatTarget(m_pPlayer->GetCID(), "Your clan name must be VeNuM to enter Clan Room.");
				m_pPlayer->m_ClanMSG = false;
			}
	}
	if((m_TileIndex == TILE_ZCLAN) || (m_TileFIndex == TILE_ZCLAN))
	{
			if(str_comp(Server()->ClientClan(m_pPlayer->GetCID()), "Zone") == 0)
			{
				if(m_pPlayer->m_ClanMSG)
				{
					GameServer()->CreateSound(m_Pos, SOUND_RIFLE_BOUNCE);
					GameServer()->SendChatTarget(m_pPlayer->GetCID(), "Welcome to Zone Clan Room..!");
					m_pPlayer->m_ClanMSG = false;
				}
			}
			else
			{
				GameServer()->CreateSound(m_Pos, SOUND_PLAYER_PAIN_LONG);
				Die(m_pPlayer->GetCID(), WEAPON_WORLD);
				GameServer()->SendChatTarget(m_pPlayer->GetCID(), "Your clan name must be Zone to enter Clan Room.");
				m_pPlayer->m_ClanMSG = false;
			}
	}
	//SOUND
	if((m_TileIndex == TILE_SOUND) || (m_TileFIndex == TILE_SOUND))
	{
		GameServer()->CreateSoundGlobal(SOUND_GUN_FIRE);
		GameServer()->CreateSoundGlobal(SOUND_GRENADE_EXPLODE);
		char aBuf[256];
			str_format(aBuf, sizeof(aBuf), "You were shot by Artility :D  ... booooommm   :P", GetPlayer()->m_Authed);
			GameServer()->SendChatTarget(m_pPlayer->GetCID(), aBuf);
	}
	...
	if((m_TileIndex == TILE_MODER) || (m_TileFIndex == TILE_MODER))
	{
		//~ m_pPlayer->GetCID()
		if (GetPlayer()->m_Authed < 1)
		{
			...
			Die(m_pPlayer->GetCID(), WEAPON_WORLD);
			char aBuf[256];
			str_format(aBuf, sizeof(aBuf), "Moderators only! Your rank: %i", GetPlayer()->m_Authed);
			GameServer()->SendChatTarget(m_pPlayer->GetCID(), aBuf);
		}
		else
		{
			...
			char aBuf[256];
			str_format(aBuf, sizeof(aBuf), "Welocom to Moderators Room !", GetPlayer()->m_Authed);
			GameServer()->SendChatTarget(m_pPlayer->GetCID(), aBuf);
		}
	}
	if((m_TileIndex == TILE_MAFIA) || (m_TileFIndex == TILE_MAFIA))
	{
		if(GameServer()->m_apPlayers[GetPlayer()->GetCID()] && str_comp(GameServer()->m_apPlayers[GetPlayer()->GetCID()]->m_TeeInfos.m_SkinName, "warpaint") == 1)
		{
			if(m_pPlayer->m_ClanMSG)
			{
				GameServer()->CreateSound(m_Pos, SOUND_RIFLE_BOUNCE);
				GameServer()->SendChatTarget(m_pPlayer->GetCID(), "Welcome to Teecity  mafia  Room..!");
				m_pPlayer->m_ClanMSG = false;
			}
		}
		else
		{
			GameServer()->CreateSound(m_Pos, SOUND_PLAYER_PAIN_LONG);
			Die(m_pPlayer->GetCID(), WEAPON_WORLD);
			GameServer()->SendChatTarget(m_pPlayer->GetCID(), "You dont have  Teecity  Skin ! change your skin to Warpaint  first");
			m_pPlayer->m_ClanMSG = false;
		}
	}
